...
class TestGenerators(vmtest.VmTestCase):
    ...
    if PY3:  # PY3.3+ only
        def test_yield_from(self):
            ...
            def main():
                x = outer()
                next(x)  # Активируем генератор
                y = x.send("Hello, World")  # Отправляем значение
                print(y)

            def outer():
                yield from inner()  # Возвращаем генератор из inner()

            def inner():
                y = yield  # Ожидаем значение
                yield y  # Возвращаем полученное значение
